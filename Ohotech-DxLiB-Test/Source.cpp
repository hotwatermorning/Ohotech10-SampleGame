...
/*
	hwm::task_queueを使って、マルチスレッドで並列にデータを処理するサンプルゲーム

	参考にしたコード: http://homepage2.nifty.com/natupaji/DxLib/dxprogram.html#N24

	UseMultiThreadマクロに1を設定してコンパイルすると、Lasers::Update()関数で並列処理を行う
*/
...
static int const kGameOverTime = 30 * 60;   // フレーム
static int const kNumLaserLimit = 400;		// レーザーの数
static int const kShootInterval = 2;		// 発射の間隔(フレーム)
static int const kAfterGrowTimeLimit = 64;	// 残光の時間(フレーム)
static int const kHitBlinkTime = 120;       // レーザー衝突時の点滅時間
static int const kHitBlinkPeriod = 10;		// 点滅の間隔
static int const kTextSize = 24;			// 標準のテキストサイズ
static int const kScoreTextSize = 60;		// スコア画面のテキストサイズ
static std::string const kGameTitle = "30秒間何とか逃げるゲーム";
static int const kNumVelocityDirections = 12; // レーザーの移動方向の数
...
class Player;	//自機
class Boss;		//ボス。レーザーを一定数発射して、上限に達したら消える
class Laser;	//レーザー。自機を攻撃する
class Lasers;	//レーザーのコンテナ

//! 0.0 .. 1.0の乱数を取得
//! 本当はthread_localが使えればスレッドローカルなRandReal変数を作ってmutexを無くしたい
struct RandReal
{
	RandReal()
		:	dist_(0.0, 1.0)
...
struct Stage
{
	...
	void Draw()
	{
...

		{
			...
			DrawString(kWidth - text_width - 10, kTextSize, time.c_str(), text_color, text_edge_color); //時間を表示
		}
	}
};
...
public:
	...
	void Draw()
	{
...

		{
			...
			DrawString(10, kTextSize, fps.c_str(), text_color, text_edge_color); //fpsを表示
		}
